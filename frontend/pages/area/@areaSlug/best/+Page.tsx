    ...
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        ...
        <header className={styles.header}>
          <h1 className={styles.title}>Best Rated Food Businesses in {location.name}</h1>
          <p className={styles.subtitle}>
            {totalCount.toLocaleString()} businesses with ratings of 4 or 5 stars
          </p>
        ...
        <section className={styles.businesses}>
          {businesses.length > 0 ? (
            ...
            <p>No businesses with ratings of 4 or 5 found in {location.name}.</p>
          )}
        </section>
      </main>
      ...
      <BusinessListStructuredData location={location} businesses={businesses} type="best" />
      <BreadcrumbStructuredData
        items={[
          { name: 'Home', url: DOMAIN },
          { name: location.name, url: `${DOMAIN}/area/${location.slug}` },
          { name: 'Best Rated', url: `${DOMAIN}/area/${location.slug}/best` },
        ]}
      />
    </div>
  );
